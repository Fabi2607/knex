...
describe("MSSQL SchemaBuilder", function() {
  ...
  it('test has table', function() {
    ...
    equal(1, tableSql.length);
    expect(tableSql[0].sql).to.equal('select object_id from sys.tables where object_id = object_id(?)');
    expect(tableSql[0].bindings[0]).to.equal('[users]');
  });

  it('test has table with schema', function() {
    tableSql = client.schemaBuilder().withSchema('schema').hasTable('users').toSQL();
  ...
  it('test rename table with schema', function() {
    tableSql = client.schemaBuilder().withSchema('schema').renameTable('users', 'foo').toSQL();
  ...
  it('test has column', function() {
    ...
    equal(1, tableSql.length);
    expect(tableSql[0].sql).to.equal('select object_id from sys.columns where name = ? and object_id = object_id(?)');
  ...
  it('test has column with schema', function() {
    tableSql = client.schemaBuilder().withSchema('schema').hasColumn('users', 'foo').toSQL();
